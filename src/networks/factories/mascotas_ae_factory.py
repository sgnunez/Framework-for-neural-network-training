...
class MascotasAeFactory(NetFactory):
    ...
    def __init__(self, input_shape, optimizer, loss, test_name):
        ...
        self.latent_dim = (32, 32, 128)
        self.optimizer = optimizer
        self.loss = loss
        self.model_name = MascotasAeFactory.__name__
        self.test_name = test_name
        self.logger = Lu.get_looger("Mascotas AE Network Factory")
    ...
    def _encoder_layers(self, inputs):
        ...
        x = tf.keras.layers.Conv2D(filters=32, kernel_size=3, strides=1, padding="same", activation='relu',
                                   name="encode_conv1")(inputs)
        x = tf.keras.layers.MaxPooling2D(2, strides=2)(x)

        x = tf.keras.layers.Conv2D(filters=64, kernel_size=3, strides=1, padding='same', activation='relu',
                                   name="encode_conv2")(x)
        x = tf.keras.layers.MaxPooling2D(2, strides=2)(x)

        x = tf.keras.layers.Conv2D(filters=128, kernel_size=3, strides=1, padding='same', activation='relu',
                                   name="encode_conv3")(x)
        x = tf.keras.layers.MaxPooling2D(2, strides=2)(x)
    ...
    def _decoder_layers(self, inputs):
        ...
        x = tf.keras.layers.Conv2D(filters=16, kernel_size=3, strides=1, padding='same', activation='relu',
                                   name="encode_conv2")(x)
        x = tf.keras.layers.UpSampling2D(2)(x)

        x = tf.keras.layers.Conv2D(filters=3, kernel_size=3, strides=1, padding='same', activation='relu',
                                   name="encode_conv3")(x)
        x = tf.keras.layers.UpSampling2D(2)(x)
